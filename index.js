...
client.on('message', msg => {	
	if (msg.content === '!delete' && msg.author.id == "190958302744543232") {
		...
		for (let i = 0; i<recentAds.length;i++){
			output += "["+i+"] [**"+recentAds[i].name + "**] :: "+ recentAds[i].message.substring(0,8)+" || "+recentAds[i].receivedOn+"\n"
		}
		debugLog(output)
	}
})
...
wss.on('message', function incoming(msg) {
	...
	let enTime = "*"+psTime.toLocaleDateString('en-US')+"*  | *"+psTime.toString().replace(/^.+?[0-9]\s([0-9]?[0-9]\:[0-9][0-9]\:[0-9][0-9])\sGMT.+?$/gi, "$1")+" PST*"
...
function prettyPrintCn(data){
    ...
    data=data.replace(/(^|[^A-Za-z])(WANT TO SELL)(?=[^A-Za-z]|$)/gi, '$1__=[卖]=__');
    data=data.replace(/(^|[^A-Za-z])(WANT TO BUY)(?=[^A-Za-z]|$)/gi, '$1__=(买)=__');
    ...
    //2. 卖买
    data=data.replace(/(^|[^A-Za-z])(SELL*?I*?ING*?|WW*?TSS*?|WT\$|SELL|VENDR*?E*?|VVTS|W[^A-Za-z]*?T[^A-Za-z]*?S)(?=[^A-Za-z]|$)/gi, '$1__=[卖]=__');
    data=data.replace(/(^|[^A-Za-z])(BUYING|BUYIN|WYB|WW*?TBB*?|VVTB|ACHETE*?R*?S*?|BUY|W[^A-Za-z]*?T[^A-Za-z]*?B|WTV)(?=[^A-Za-z]|$)/gi, '$1__=(买)=__');	    
